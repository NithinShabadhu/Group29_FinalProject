...
exports.getlatestblogs = async(req,res)=>{
  ...
  if(session){
    ...
    try {
        ...
        res.render('getlatestblogs', { title: 'latest blogs', allBlogs , isAuth:isAuth,user:user  } );
      } catch (error) {
...
exports.submitNewBlog = async ( req,res,next)=>{
    ...
    if(!session){
      res.status(400).render('Unauthorized',{isAuth:false});
    }
    else{
...
exports.getBlogbyId = async(req,res) => {
  ...
  try {
    ...
    if(ObjectId.isValid(blogId)){
      ...
      res.render('blog', { title: 'Blog by id ', blog : sendblog , comments : Allcomments , isAuth: isAuth , user:user} );
    }
    else{
   ...
   // const updateLikes = await Blogs.updateOne({_id : blogid} ,{$push:{likedUsers : liked_users}})
    ...
    if(updatedBlog.modifiedCount != 0){
      const getBlog = await Blogs.findById(blogid);
       ...
       console.log("likes count -======"+ getBlog.likes_count);
